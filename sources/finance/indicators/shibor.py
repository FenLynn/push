...
class ShiborIndicator(BaseIndicator):
    ...
    def plot(self, df: pd.DataFrame) -> str:
        ...
        # Colors: Darken ON gray
        c_on = '#636e72'   # Darker Gray (User Request)
        c_3m = '#3498DB'   # Blue
        c_1y = '#8e44ad'   # Purple
        
        # Top: Recent
        ax_top = axes[0]
        ax_top.plot(df_short['date'], df_short['ON'], color=c_on, linewidth=1.2, label='隔夜', alpha=0.8)
        ax_top.plot(df_short['date'], df_short['3M'], color=c_3m, linewidth=3, label='3月')
        ax_top.plot(df_short['date'], df_short['1Y'], color=c_1y, linewidth=2.5, label='1年')
        
        self.plotter.draw_current_line(df_short['3M'].iloc[-1], ax_top, c_3m)
        
        self.plotter.fmt_single(fig, ax_top, title='流动性-Shibor利率 (近期13月)', ylabel='利率(%)', rotation=45,
                              data=[df_short['ON'], df_short['3M'], df_short['1Y']])
        self.plotter.set_no_margins(ax_top)
        
        # Bottom: Long
        ax_bot = axes[1]
        ax_bot.plot(df_long['date'], df_long['ON'], color=c_on, linewidth=0.5, alpha=0.4)
        ax_bot.plot(df_long['date'], df_long['3M'], color=c_3m, linewidth=1.5)
